...
4. Configure the Java process with `FIREBASE_PROJECT_ID` for that same project and Application Default Credentials. For local development, set `GOOGLE_APPLICATION_CREDENTIALS` to an absolute path to your Admin service-account JSON stored outside this project. In managed hosting, use the platform's supported service identity. Do not put the Admin JSON in frontend files, public assets, environment variables prefixed with `VITE_`, or the repository. See [Firebase Admin setup](https://firebase.google.com/docs/admin/setup).
